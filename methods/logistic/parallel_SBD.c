...
void DQNParallel_SBD(int N, int Dim, int size_row, int size_col, char* type, char* path){
	int my_rank;
	int procs;
	int N_w=size_col;
	int N_f, rem=0;
	...
	double gradient_local_time_start, gradient_local_time_end, gradient_local_total=0.0;
	double hesian_local_time_start, hesian_local_time_end, hesian_local_total=0.0;
	double check_local_time_start, check_local_time_end, check_local_total=0.0;
	double calc_local_time_start, calc_local_time_end, calc_local_total=0.0;
	...
	for(int k=iter;k>0;--k){
	  ...
	  compute_Gradient(N_w, N_x, N_f, rem, Dim, X, Gradient, GradOld, my_rank, Adata, Bdata, lambda_penal);
		...
		if(active[my_rank]){
	   	...
	   	GradientGlob=calloc(Dim*N, sizeof(double));
	  ...
	  cblas_daxpy(Dim, -0.2, sDirection, 1, X, 1);
